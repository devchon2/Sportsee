...
export default async function getProfile(id, source) {
  ...
  if (source === 'mock') {
    Datas = [
      ...
      mockDatas.USER_ACTIVITY.filter((obj) => Number(obj.userId) === Number(id))[0],
      mockDatas.USER_AVERAGE_SESSIONS.filter((obj) => Number(obj.userId) === Number(id))[0],
    ]
  } else {
    ...
    Datas = [ await infos.data, await perfs.data, await activities.data, await average.data];
  }
