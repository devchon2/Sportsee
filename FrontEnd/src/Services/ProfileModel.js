...
export default class ProfileModel {
  constructor(id, userDatas) {
    ...
    this.formatedActivities = [
      { kind: this.activities.kind[6] === 'intensity' ? 'Intensité' : null, data: (this.activities.data[5]).value },
      { kind: this.activities.kind[5] === 'speed' ? 'Vitesse' : null, data: (this.activities.data[4]).value },
      { kind: this.activities.kind[4] === 'strength' ? 'Force' : null, data: (this.activities.data[3]).value },
      { kind: this.activities.kind[3] === 'endurance' ? 'Endurance' : null, data: (this.activities.data[2]).value },
